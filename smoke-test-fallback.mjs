#!/usr/bin/env node
// smoke-test-fallback.mjs — Integration test for adaptive fallback
// Run: node smoke-test-fallback.mjs
...
async function main() {
  ...
  const tracker = new TokenTracker(config.tracking?.log_file || '.llmapi_usage.json');
